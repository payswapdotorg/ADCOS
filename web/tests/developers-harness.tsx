/**
 * The shared component-test harness for the Developers-area suites
 * (the same pattern Worker 1's shell suite and Worker 2's feature
 * suites established): fetch is mocked at the GLOBAL boundary with a
 * router over REAL fixture envelopes; the page under test renders
 * inside the REAL SessionProvider, and `renderWithSession` drives the
 * real connect() flow (GET /api/2.0/application) so the session's
 * client carries the exact auth headers the backend expects. NO
 * network, ever.
 *
...
import { FIXTURE_APPLICATION, FIXTURE_SESSION } from "./developers-fixtures";
...
export function envelope<T>(data: T): unknown {
  return {
    ...
    rate_limit: { limit: 1000, remaining: 999, reset_at: "2026-09-14T00:00:01Z" },
  };
}

/** The typed error envelope shape every /api/2.0 failure returns. */
export function errorEnvelope(reason: string, message: string, status: number): unknown {
  return {
    api_version: "2.0",
    environment: "sandbox",
    request_id: "sha256:harness_error_id",
    error: {
      canonical_reason: "",
      environment: "sandbox",
      http_status: status,
 ...
 * route key ends with `*`). Handlers return the raw body (or a Response,
 * or `{__status, body}`), and FUNCTION handlers compute the body per
 * call (counters, mutable state). Unmatched routes fail the test loudly.
 */
export function routeFetch(
  ...
  const fn = ((path: string, init?: RequestInit) => {
    ...
    // function handlers compute the body/response per call
...
export function renderWithSession(
  ui: ReactNode,
  fetchImpl?: (path: string, init?: RequestInit) => unknown,
) {
  function Connector() {
 ...
 * requests the page made — method, path, headers and body.
 */
export function trackedFetch(routes: Record<string, unknown>) {
