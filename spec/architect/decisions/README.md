...
| DEC-0084 | governance | null | ACCEPTED | ACCEPTED | R1 governance reconciliation after the accepted R0 restoration (PR #8, merge a3391e8): reconciled roadmap (Version 1.1), current-state, execution-state, and execution-ledger (LEDGER-RECON-012) to the restored mainline; W044–W049 and W050 lifecycle projections reconciled to their existing durable acceptance history with original provenance preserved; exactly zero active authorizations; R1 complete; next gate R2; historical decisions preserved; the historical R1 records in Git history on the former repository lineage are neither imported nor replayed and are not the current baseline |
