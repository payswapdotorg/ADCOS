...
- `ACR-014-architecture-1.1-freeze.md` — PROPOSED (DEC-0098 approval path): adopts Architecture 1.1 as the sole normative forward architecture, promotes the 1.1 architecture/lock/work-item/dependency successor snapshots into the canonical authority files, preserves the Architecture 1.0 snapshot verbatim under `spec/history/`, accepts the migration classification matrix and vertical-proof boundaries, and records the resulting Architecture Version 1.1. Synchronized updates land with the M001 delivery (authorization M001-CORE-001); ACR-014 becomes ACCEPTED at the DEC-0099 acceptance.
