"""The M012 attributable evidence ledger.

The frozen COMOS flow's evidence must remain attributable to THE
contract — including the usage evidence the REAL M009 usage
authority produces, the assurance evaluations the M005 seam
observes, the step-4 requirements submission, and the step-2
selection decision.  This module is the harness's typed,
fail-closed attribution surface:

- **Evidence typing (LOCK-106).**  Usage observations, assurance
  evaluations, requirements submissions, and selection decisions
  are DISTINCT evidence kinds — a usage record is never an
  assurance record, and none of them is a claim, a commitment, or
  an attestation.
- **Attribution is structural (LOCK-117/LOCK-118).**  Every entry
  carries the canonical contract id, the recorded instant, the
  issuer, and decision references.  The ledger is BOUND to the
  real ``ContractStore`` (read-only public projection) and FAILS
  CLOSED: an entry whose contract does not exist in the canonical
  fold is rejected; an unbound ledger rejects every write.
- **Data, never authority.**  The ledger never mutates a contract,
  never confers selection, and never promotes an observation into
  an attestation.  Canonical JSON round-trips are exact.
- **The usage entries cite the REAL authority.**  A
  ``usage-observation`` entry's token IS the observation id the
  accepted M009 ``UsageLedger`` derived — the attribution record
  is the bridge between the authority's own journal and this
  ledger's contract attribution (DATA read through the authority's
  public surface, never a re-implementation of it).
...
from .application import REASON_NOT_BOUND, REASON_VOCABULARY, ComosError
...
#: types — the usage observations the real M009 authority produces,
#: the assurance evaluations the M005 seam observes, the step-4
#: requirements submission, and the step-2 selection decision;
#: never claims, commitments, or attestations).
...
#: The ledger issuer identities (who produced each entry: the real
#: M009 usage authority, the M005 seam, the COMOS application, the
#: M004 seam; each entry's provenance is part of its attribution).
...
@dataclass(frozen=True)
class Attribution:
    """The attribution record for one contract (the proof artifact):
    every evidence entry attributable to THE contract, with the
    per-kind counts and the canonical state at attribution time."""

    contract_id: str
    contract_state: str
    usage_entries: Tuple[EvidenceEntry, ...]
    assurance_entries: Tuple[EvidenceEntry, ...]
    requirements_entries: Tuple[EvidenceEntry, ...]
    selection_entries: Tuple[EvidenceEntry, ...]
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "requirements_count": len(self.requirements_entries),
            "selection_count": len(self.selection_entries),
            "usage_tokens": [entry.token for entry in self.usage_entries],
            "assurance_tokens": [
                entry.token for entry in self.assurance_entries
            ],
            "requirements_tokens": [
                entry.token for entry in self.requirements_entries
            ],
            "selection_tokens": [
                entry.token for entry in self.selection_entries
            ],
        }
...
class EvidenceLedger:
    ...
    def requirements_entries(self, contract_id: str) -> Tuple[EvidenceEntry, ...]:
        return tuple(
            entry
            for entry in self.for_contract(contract_id)
            if entry.kind == "requirements-submission"
        )

    def selection_entries(self, contract_id: str) -> Tuple[EvidenceEntry, ...]:
        return tuple(
            entry
            for entry in self.for_contract(contract_id)
            if entry.kind == "selection-decision"
        )

    def attribution(self, contract_id: str) -> Attribution:
        """The proof artifact: every entry attributable to THE
        ...
        return Attribution(
            ...
            requirements_entries=self.requirements_entries(contract_id),
            selection_entries=self.selection_entries(contract_id),
        )
