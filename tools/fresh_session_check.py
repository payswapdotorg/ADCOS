...
def main() -> int:
    ...
    required_markers = [
        ...
        (roadmap, 'roadmap_version: "2.1"', "roadmap must be advanced to the post-M009-acceptance version 2.1"),
    ...
    for marker in ["R6 Provider Onboarding & Federation", "M001 — Architecture 1.1 Freeze", "the sole active implementation authorization", "Architecture 1.0 is preserved historical evidence", "R7 — Universal Connectivity Commerce", "M002 — Connectivity Contract Core", "M003 — Offers and Provider Capability Exchange", "M013 — Developer Connectivity API", "M009 — Usage and Commercial Reconciliation", "M004 — Eligibility and Policy"]:
        if marker.lower() not in current.lower():
            failures.append(f"current-state.md missing R7-active checkpoint marker: {marker}")

    for marker in ['roadmap_version: "2.1"', "program_state: R7_UNIVERSAL_CONNECTIVITY_COMMERCE_ACTIVE", "execution_mode: implementing", "active_work_item: M004", "active_authorization: R7-CORE-001", "next_gate: R7_UNIVERSAL_CONNECTIVITY_COMMERCE", "completion_decision: DEC-0100", "activation_decision: DEC-0101"]:
        if marker not in roadmap:
            failures.append(f"roadmap.yaml missing current authoritative marker: {marker}")

    for marker in ["mode: implementing", "active_work_item: M004", "active_authorization: R7-CORE-001", "current_child_work_item: M004", "program_authorization: R7-CORE-001", "R7-CORE-001"]:
