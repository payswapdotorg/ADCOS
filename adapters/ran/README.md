...
## RF-simulation validation environment (Architect work order, PR #21)

`adapters/ran/rfsim.py` is the INDEPENDENTLY implemented OAI-RFsim-style gNB/UE emulation environment added by the Architect-authorized RF-simulation validation phase: a real REST-over-HTTP RAN control-plane peer (`RfSimRanPeer`) whose control-plane answers are derived from a deterministic RADIO-CHANNEL MODEL (`RfSimScenario`/`RfSimEnvironment`) — the RF analogue of OpenAirInterface's `--rfsim` mode (the RF front end replaced by a simulated channel between virtual gNB and UE):

- **Channel model (all integer, all frozen constants):** TR 38.901 UMa NLOS path-loss anchor table (3.5 GHz, milli-dB), TR 38.901 shadowing (sigma=6 dB discretized over ±6 dB in 0.5 dB steps) and per-transmission fast fading (±3 dB), per-PRB thermal noise (-111437 milli-dBm at 30 kHz SCS + 7 dB NF), inter-cell interference combined in the dB domain via a frozen dB-addition table, and a TS 38.214-inspired SINR → MCS ladder that drives **admission** (received-power floor + mcs-1 SINR floor), **PRB demand** (1 PRB at healthy SINR — the reference engine's accounting; more as the channel degrades), **health** (DEGRADED when a live bearer's SINR falls below the healthy floor), and **per-transmission decode success** (the current SINR with a seeded fading draw must still support the admitted MCS, else a typed 503 — never a corrupted echo).
- **Independence (structural, not cosmetic):** a separate state machine and HTTP plumbing importing NOTHING from `conformance.py` — only the family vocabularies (`model.py`, `validation.py`, `serialization.py`) and the engine's ref-minting helpers (`_mint_ref`/`FIRST_RNTI`/`LAST_RNTI`/`RAN_ALLOCATION_KINDS`, the exact precedent set the conformance peer itself imports, so identical operation histories mint identical refs and canonical manager state stays byte-identical across engine / conformance peer / RF-sim peer). Its behavior is radio-derived: coverage-driven admission, channel-derived PRB demand, SINR-conditioned delivery, GEOMETRIC serving-cell selection — outcomes the registry-only conformance peer cannot produce (battery cases 37/40/46 prove the difference).
- **Determinism:** every draw is a PURE content-addressed function of (seed, label) — sha256 with rejection sampling, no shared mutable stream — so repeated queries are stable and the same scenario replays byte-identically (cases 33/44).
- **HONESTLY DISCLOSED:** an RF channel SIMULATION is not an SDR and not a real RAN stack. `rf_simulation`/`rfsim` are FORBIDDEN peer kinds in the interop gate (case_42: FORBIDDEN fires BEFORE any network probe), and the frozen SDR-based-lab acceptance criterion remains untouched — see `docs/WORK-020-rf-simulation-evidence.md` for the four-class evidence classification.
...
- `RAN_INTEROP` unset → **SKIP** with a transparent gate-disabled disclosure (the conformance suite plus the RF-simulation validation environment remain the strongest honest in-sandbox evidence — neither can satisfy the frozen SDR criterion).
- `RAN_INTEROP=1` + `RAN_PEER_KIND` asserting a non-real peer (`reference|inrepo|conformance_server|simulator|rf_simulation|rfsim`) → **FORBIDDEN** before any network probe — the anti-faking rule: the in-repo conformance peer, any simulator, and the RF-simulation environment can NEVER satisfy the SDR-based-lab criterion, not even as a fallback.
...
Real RAN interoperability evidence in three layers:
...
2. **RF-simulation validation environment (always runs, cases 33-46):** the independent OAI-RFsim-style channel-model peer exercising the SAME production adapter boundary with radio-driven admission/PRB/health/decode behavior, deterministic evidence, and three-way implementation-substitution invariance — RF-simulation interoperability evidence that is explicitly NOT SDR evidence (see `docs/WORK-020-rf-simulation-evidence.md`).
3. **Real SDR-lab interop gate (environment-gated by `RAN_INTEROP=1`):** exercises the full byte-path against a REAL OpenAirInterface/O-RAN lab on real SDR hardware (capabilities/state → gNB provisioned + cell ACTIVE on the real SDR → UE attach → DRB bound to the ADCOS session_id → payload byte-identical). Closes the frozen SDR-lab criterion when the lab is reachable; reports UNREACHABLE with the explicit capability matrix when it is not (no in-repo simulator fallback), and FORBIDDEN before any network probe on a forbidden peer-kind assertion.
